...
import { Plus, Search, Edit2, Trash2, Package, Tag, Layers, RefreshCw, Barcode, AlertCircle } from 'lucide-react';
...
export default function Products({ products, onAddProduct, onUpdateProduct, onDeleteProduct, formatMoney }: ProductsProps) {
                  ...
                  <div className="flex-1 min-w-0 space-y-1">
                    ...
                    <div className="flex items-end justify-between pt-1">
                      ...
                      <div className="text-right space-y-1.5">
                        <span className={`inline-flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-full font-bold ${isLowStock ? 'bg-amber-50 dark:bg-amber-950/20 text-amber-600' : 'bg-emerald-50 dark:bg-emerald-950/20 text-emerald-600'}`}>
                          <span className={`w-1.5 h-1.5 rounded-full ${isLowStock ? 'bg-amber-500' : 'bg-emerald-500'}`}></span>
                          {p.quantity} {isLowStock ? (language === 'sw' ? 'Pungufu' : 'Low Stock') : (language === 'sw' ? 'Ipo' : 'In Stock')}
                        </span>
                    ...
                    <tr key={p.id} className="hover:bg-zinc-50/40 dark:hover:bg-zinc-800/10 transition-colors">
                      ...
                      <td className="p-4 whitespace-nowrap">
                        <div className="flex items-center gap-1.5">
                          <span className={`w-2.5 h-2.5 rounded-full ${isLowStock ? 'bg-amber-500' : 'bg-emerald-500'}`}></span>
                          <span className={`font-semibold ${isLowStock ? 'text-amber-600 dark:text-amber-400 font-bold' : 'text-zinc-700 dark:text-zinc-300'}`}>
                            {p.quantity} {isLowStock && (language === 'sw' ? '(Pungufu!)' : '(Low!)')}
                          </span>
                        </div>
                      </td>
